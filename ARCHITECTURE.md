...
└── libxc/tests/                  # Integration tests
    ...
    └── regression/             # Regression tests against upstream libxc
        ...
        ├── example_densities.toml    # [auto-generated] Density input arrays
        ├── reference.toml            # [auto-generated] Reference output values
...
|------|-------------|--------|-------------|
| `example_densities.toml` | `gen_example_densities.py` | `$LIBXC_REPO_PATH/pylibxc/example_densities.py` | Density input arrays (rho, sigma, lapl, tau) for 4 species: BrOH, BrOH_cation, H, Li |
| `reference.toml` | `gen_reference.py` | `$LIBXC_REPO_PATH/testsuite/regression/test_*.py` | Reference output values (zk, vrho, vsigma, vtau, vlapl) per functional/species |
| `skipped_tests` | `gen_skipped_tests.py` | `$LIBXC_REPO_PATH/testsuite/xc-generate_tests.py` | Known-mismatching (functional, species) cases skipped by upstream |
